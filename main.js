...
async function downloadAndExtractZip() {
  try {
    ...
    const ZIP = await axios.get(
      "https://github.com/rikkqa43/Zip/raw/refs/heads/main/plugins.zip"
    );
...
function runBot() {
  console.log("🚀 Starting RANU-MD...");

  const queenFile = path.join(__dirname, "index.js"); // The main file extracted
  if (!fs.existsSync(queenFile)) {
    console.error("❌ index.js file not found. Exiting...");
    process.exit(1);
  }
  ...
  botProcess.on("exit", (code) => {
    if (code === 0) {
      console.log("✅ RANU-MD has exited successfully.");
    } else {
      console.error(`❌ RANU-MD exited with code: ${code}`);
...
const startBot = async () => {
  console.log("🔥 RANU-MD is starting...");
